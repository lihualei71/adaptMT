#----------------------------------------------------------------
# This script calculates p-values on GEOquery Data: The code is from Ang Li and
# Rina Foygel Barber (2016) 'Multiple testing with the structure adaptive
# Benjamini-Hochberg algorithm' We take the first part of the source code from
# 'http://www.stat.uchicago.edu/~rina/sabha/gene_drug_data_example.R' and
# modified it for our purposes.  The p-values are stored in
# 'inst/extdata/GEOquery.RData'
#----------------------------------------------------------------
...
gene_drug_get_pvals <- function(Data, highdose = 21:25, lowdose = 6:10, control = 1:5) {
    ...
    ord <- order(abs(pvals_highdose))
    ord_small <- order(abs(pvals_highdose_small))
...
save(file = "../extdata/estrogen.RData", estrogen)
